...
- src/services：业务服务（模拟面试服务、邮件解析服务等）
...
P["pdfjs-dist<br/>PDF解析"]
O["openai<br/>AI能力"]
EP["email-parser<br/>邮件解析"]
ER["email-reply-generator<br/>邮件回复生成"]
SR["spot-recommendation-service<br/>景点推荐"]
MI["mock-interview-service<br/>模拟面试"]
end
B --> A
A --> D
D --> C
...
- 服务层：封装业务逻辑（如模拟面试服务、邮件解析服务），屏蔽第三方 API 细节。
...
- LearnPage：新增文档处理功能，包括Word文档解析和PDF处理状态指示器。
- InboxPage：邮件解析与回复生成功能。
...
- 文档处理状态：新增 PDF 解析状态指示器样式，包括加载动画和成功状态样式。
...
C --> E["文档处理状态<br/>.pdf-parsing-status/.pdf-parsing-spinner"]
```

图表来源
- [src/index.css:1-46](file://src/index.css#L1-L46)
- [src/App.css:1-800](file://src/App.css#L1-L800)
- [src/App.css:5086-5118](file://src/App.css#L5086-L5118)
...
- 文档处理体验：Word 文档解析提供实时状态指示，包括加载动画和成功提示。
...
- [src/pages/LearnPage.js:265-298](file://src/pages/LearnPage.js#L265-L298)
...
- pdfjs-dist：PDF 解析与文档处理能力。
- openai：AI 能力集成（用于聊天功能）。
- 业务服务：EmailParser、EmailReplyGenerator、SpotRecommendationService、MockInterviewService。
...
EMAIL["email-parser"] --> SERVICES
REPLY["email-reply-generator"] --> SERVICES
SPOT["spot-recommendation-service"] --> SERVICES
INTERVIEW["mock-interview-service"] --> SERVICES
```
...
- 文档处理优化：Word 文档解析使用 FormData 上传，避免大文件内存占用。
...
- 文档解析失败：检查 LearnPage 中文件格式验证和后端 API 调用，确认 Token 有效性。
- PDF 状态指示异常：检查 .pdf-parsing-status 样式类是否正确应用。
...
## 结论
本架构以 React 18 与 React Router DOM 6 为基础，采用 App Shell 模式与单页应用（SPA）结构，通过组件化设计与公共组件复用，实现清晰的页面层次与一致的交互体验。全局状态集中在 App 壳层，服务层封装复杂业务流程，样式系统通过 CSS 变量与前缀化类名实现可维护性与可扩展性。新增的文档处理组件（Word 文档解析器和 PDF 处理状态指示器）进一步增强了应用的功能完整性。建议后续引入路由懒加载与状态管理库以进一步提升性能与可维护性。
...
- 新增功能：LearnPage 中的文档处理功能，包括 Word 文档解析和 PDF 状态指示器。
- 服务层扩展：MockInterviewService、EmailParser、EmailReplyGenerator、SpotRecommendationService 等业务服务。
