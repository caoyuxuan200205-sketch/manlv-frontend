...
## 引言
本文件为漫旅 ManLv 的"情景学习引擎"提供系统化技术文档，覆盖地点推荐算法、城市导览、学习内容推送、PDF/Word 解析服务、高德地图集成、地理围栏与实时位置服务的实现思路与扩展方案。文档面向开发者与产品/运营人员，既提供代码级细节，也提供概念性架构与流程图，帮助快速理解与二次开发。

## 项目结构
前端采用 React 18 + React Router DOM，页面组件位于 src/pages，通用服务位于 src/services，公共组件位于 src/components。核心页面包括：AuthPage、HomePage、SchedulePage、TripDetailPage、LearnPage（情景学习）、InboxPage（邮件解析）、ProfilePage、ChatPage（AI 对话/模拟面试）。服务层包含地点推荐、简历解析（支持PDF/Word）、邮件解析与回复生成、模拟面试等模块。
...
- [src/pages/LearnPage.js:1-670](file://src/pages/LearnPage.js#L1-L670)
...
- PDF/Word 解析服务：封装简历上传与解析调用，支持PDF、TXT、MD、DOC、DOCX格式，包含完整的错误处理和用户反馈机制。
...
ENDPOINT["http://localhost:3001/api/parse-resume"]
end
LP --> SRV1
LP --> SRV2
LP --> SRV5
...
SRV->>ARK : POST /chat/completions + system prompt + user
ARK-->>SRV : JSON 地点数组
SRV-->>LP : 推荐结果
...
### PDF/Word 解析服务集成
- 功能：支持多种格式简历上传（PDF、TXT、MD、DOC、DOCX），调用后端 /api/parse-resume，返回解析结果（文本、结构化信息、扫描版提示）
- 格式校验：支持 MIME 类型与扩展名校验，格式不符提示
- Word 文档解析：通过 http://localhost:3001/api/parse-resume API 处理 DOC/DOCX 文件，包含完整的错误处理和用户反馈机制
- 状态管理：解析中显示加载动画，解析完成显示字符数统计，失败显示错误信息
- 交互：支持清除并重新上传，登录状态检查与授权头处理

**更新** 新增 Word 文档解析功能，支持 .doc 和 .docx 格式文件的上传和解析

```mermaid
sequenceDiagram
participant U as "用户"
participant LP as "LearnPage"
participant API as "后端 /api/parse-resume"
U->>LP : 选择简历文件支持 .doc/.docx/.pdf/.txt/.md
LP->>LP : 格式校验isTextFile/isDocFile
alt 文本文件
LP->>LP : 直接读取文件内容
else Word 文档
LP->>LP : 检查登录状态localStorage.manlv_token
LP->>API : POST multipart/form-data + Authorization : Bearer token
API-->>LP : { data : { text : "...", structured : {...} }, message }
end
LP->>LP : 更新 UI 状态解析成功/失败提示
```

图表来源
- [src/pages/LearnPage.js:226-300](file://src/pages/LearnPage.js#L226-L300)
- [src/pages/LearnPage.js:526-579](file://src/pages/LearnPage.js#L526-L579)
...
MI->>ARK : POST /chat/completions + system prompt + first question
ARK-->>MI : 首个问题
...
MI->>ARK : POST /chat/completions + 完整历史
ARK-->>MI : 反馈 + 下一个问题
MI-->>CP : 反馈
end
U->>CP : 结束面试
CP->>MI : finishInterview(sessionId)
MI->>ARK : POST /chat/completions + summary prompt
ARK-->>MI : 结构化评分 JSON
MI-->>CP : 评分与反馈
```
...
- PDF/Word 解析：大文件解析耗时较长，建议在 UI 上提供进度条与取消能力；Word 文档解析需要网络请求和授权头处理
...
- PDF/Word 解析异常
  - 现象：解析失败或提示扫描版
  - 排查：确认文件格式与大小；检查后端 /api/parse-resume API 可访问性；查看后端返回的错误信息；尝试重新上传
- Word 文档解析特定问题
  - 现象：Word 解析失败或提示"未能提取文档内容"
  - 排查：确认文件为 .doc 或 .docx 格式；检查本地存储的 manlv_token 是否存在；验证 http://localhost:3001/api/parse-resume API 可用性；查看网络面板中的请求状态
- AI 对话无响应
  - 现象：SSE 无输出或报错
...
## 结论
情景学习引擎以"城市×专业"矩阵为核心，结合 AI 推荐、地图导览与模拟面试，构建了从"地点推荐—城市导览—学习推送—实战演练"的闭环。当前实现已具备稳定的推荐与面试能力，新增的 Word 文档解析功能进一步完善了简历处理能力。建议后续在地理围栏、实时位置、学习内容个性化推送与数据持久化方面持续演进，以进一步提升用户体验与学习成效。
...
- Word 文档解析测试：参见 [TESTING_GUIDE.md](file://TESTING_GUIDE.md)，包含完整的测试步骤和故障排查指南